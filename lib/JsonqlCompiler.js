var JsonqlCompiler, QueryOptimizer, SqlFragment, _, isInt,
  indexOf = [].indexOf || function(item) { for (var i = 0, l = this.length; i < l; i++) { if (i in this && this[i] === item) return i; } return -1; };

_ = require('lodash');

SqlFragment = require('./SqlFragment');

QueryOptimizer = require('./QueryOptimizer');

module.exports = JsonqlCompiler = (function() {
  function JsonqlCompiler(schemaMap, optimizeQueries) {
    if (optimizeQueries == null) {
      optimizeQueries = false;
    }
    this.schemaMap = schemaMap;
    this.nextId = 1;
    this.optimizeQueries = optimizeQueries;
  }

  JsonqlCompiler.prototype.compileQuery = function(query, aliases, ctes) {
    var f, frag, from, i, len, ref, selects, w, where, withClauses;
    if (aliases == null) {
      aliases = {};
    }
    if (ctes == null) {
      ctes = {};
    }
    if (query.type === "union") {
      return SqlFragment.join(_.map(query.queries, (function(_this) {
        return function(q) {
          return new SqlFragment("(").append(_this.compileQuery(q, aliases, ctes)).append(")");
        };
      })(this)), " union ");
    }
    if (query.type === "union all") {
      return SqlFragment.join(_.map(query.queries, (function(_this) {
        return function(q) {
          return new SqlFragment("(").append(_this.compileQuery(q, aliases, ctes)).append(")");
        };
      })(this)), " union all ");
    }
    if (this.optimizeQueries) {
      query = new QueryOptimizer().optimizeQuery(query);
    }
    frag = new SqlFragment();
    aliases = _.clone(aliases);
    ctes = _.clone(ctes);
    if (query.withs && query.withs.length > 0) {
      withClauses = [];
      ref = query.withs;
      for (i = 0, len = ref.length; i < len; i++) {
        w = ref[i];
        f = new SqlFragment('"').append(this.schemaMap.mapTableAlias(w.alias));
        f.append("\" as (");
        f.append(this.compileQuery(w.query, aliases));
        f.append(")");
        withClauses.push(f);
        if (ctes[w.alias]) {
          throw new Error("CTE alias " + w.alias + " in use");
        }
        ctes[w.alias] = true;
      }
      frag.append("with ");
      frag.append(SqlFragment.join(withClauses, ", "));
      frag.append(" ");
    }
    frag.append('select ');
    if (query.distinct) {
      frag.append('distinct ');
    }
    if (query.from) {
      from = this.compileFrom(query.from, aliases, ctes);
    } else {
      from = null;
    }
    selects = _.map(query.selects, (function(_this) {
      return function(s) {
        return _this.compileSelect(s, aliases, ctes);
      };
    })(this));
    if (selects.length === 0) {
      frag.append("null");
    } else {
      frag.append(SqlFragment.join(selects, ", "));
    }
    if (from) {
      frag.append(" from ");
      frag.append(from);
    }
    if (query.where) {
      where = this.compileExpr(query.where, aliases, ctes);
      if (!where.isEmpty()) {
        frag.append(" where ");
        frag.append(where);
      }
    }
    if (query.groupBy) {
      if (query.groupBy.length > 0) {
        frag.append(" group by ");
      }
      if (!_.isArray(query.groupBy)) {
        throw new Error("Invalid groupBy");
      }
      frag.append(SqlFragment.join(_.map(query.groupBy, (function(_this) {
        return function(groupBy) {
          if (isInt(groupBy)) {
            return new SqlFragment("" + groupBy);
          }
          return _this.compileExpr(groupBy, aliases, ctes);
        };
      })(this)), ", "));
    }
    if (query.orderBy) {
      frag.append(this.compileOrderBy(query.orderBy, aliases));
    }
    if (query.limit != null) {
      if (!isInt(query.limit)) {
        throw new Error("Invalid limit");
      }
      frag.append(" limit ").append(new SqlFragment("?", [query.limit]));
    }
    if (query.offset != null) {
      if (!isInt(query.offset)) {
        throw new Error("Invalid offset");
      }
      frag.append(" offset ").append(new SqlFragment("?", [query.offset]));
    }
    return frag;
  };

  JsonqlCompiler.prototype.compileSelect = function(select, aliases, ctes) {
    var expr, frag;
    if (ctes == null) {
      ctes = {};
    }
    expr = select.expr;
    if (select.over) {
      expr = _.extend({}, expr, {
        over: select.over
      });
    }
    frag = this.compileExpr(expr, aliases, ctes);
    frag.append(" as ");
    this.validateAlias(select.alias);
    frag.append('"' + select.alias + '"');
    return frag;
  };

  JsonqlCompiler.prototype.compileFrom = function(from, aliases, ctes) {
    var fields, left, onSql, ref, right, subexpr, subquery;
    if (aliases == null) {
      aliases = {};
    }
    if (ctes == null) {
      ctes = {};
    }
    switch (from.type) {
      case "table":
        this.validateAlias(from.alias);
        if (ctes[from.table]) {
          aliases[from.alias] = true;
          return new SqlFragment(this.schemaMap.mapTableAlias(from.table)).append(' as "').append(this.schemaMap.mapTableAlias(from.alias)).append('"');
        }
        aliases[from.alias] = from.table;
        return this.schemaMap.mapTable(from.table).append(new SqlFragment(' as "' + this.schemaMap.mapTableAlias(from.alias) + '"'));
      case "join":
        left = this.compileFrom(from.left, aliases, ctes);
        right = this.compileFrom(from.right, aliases, ctes);
        if (_.intersection(_.keys(left.aliases), _.keys(right.aliases)).length > 0) {
          throw new Error("Duplicate aliases");
        }
        _.extend(aliases, left.aliases);
        _.extend(aliases, right.aliases);
        onSql = this.compileExpr(from.on, aliases, ctes);
        if ((ref = from.kind) !== 'inner' && ref !== 'left' && ref !== 'right') {
          throw new Error("Unsupported join kind " + from.kind);
        }
        return new SqlFragment("(").append(left).append(" " + from.kind + " join ").append(right).append(" on ").append(onSql).append(")");
      case "subquery":
        this.validateAlias(from.alias);
        if (aliases[from.alias] != null) {
          throw new Error("Alias " + from.alias + " in use");
        }
        subquery = this.compileQuery(from.query, aliases, ctes);
        fields = _.map(from.query.selects, function(s) {
          return s.alias;
        });
        aliases[from.alias] = true;
        return new SqlFragment("(").append(subquery).append(') as "').append(this.schemaMap.mapTableAlias(from.alias)).append('"');
      case "subexpr":
        this.validateAlias(from.alias);
        if (aliases[from.alias] != null) {
          throw new Error("Alias " + from.alias + " in use");
        }
        subexpr = this.compileExpr(from.expr, aliases, ctes);
        aliases[from.alias] = true;
        return subexpr.append(' as "').append(this.schemaMap.mapTableAlias(from.alias)).append('"');
      default:
        throw new Error("Unsupported type " + from.type + " in " + (JSON.stringify(from)));
    }
  };

  JsonqlCompiler.prototype.compileOrderBy = function(orderBy, aliases) {
    var frag;
    frag = new SqlFragment();
    if (!_.isArray(orderBy)) {
      throw new Error("Invalid orderBy");
    }
    if (!_.all(orderBy, (function(_this) {
      return function(o) {
        var ref;
        if (!isInt(o.ordinal) && !o.expr) {
          return false;
        }
        return (o.direction == null) || ((ref = o.direction) === 'asc' || ref === 'desc');
      };
    })(this))) {
      throw new Error("Invalid orderBy");
    }
    if (orderBy.length > 0) {
      frag.append(" order by ").append(SqlFragment.join(_.map(orderBy, (function(_this) {
        return function(o) {
          var f, ref;
          if (_.isNumber(o.ordinal)) {
            f = new SqlFragment("" + o.ordinal);
          } else {
            f = _this.compileExpr(o.expr, aliases);
          }
          if (o.direction) {
            f.append(" " + o.direction);
          }
          if (o.nulls && ((ref = o.nulls) === 'first' || ref === 'last')) {
            f.append(" nulls " + o.nulls);
          }
          return f;
        };
      })(this)), ", "));
    }
    return frag;
  };

  JsonqlCompiler.prototype.compileExpr = function(expr, aliases, ctes) {
    var ref, ref1;
    if (ctes == null) {
      ctes = {};
    }
    if (aliases == null) {
      throw new Error("Missing aliases");
    }
    if (expr == null) {
      return new SqlFragment("null");
    }
    if ((ref = typeof expr) === "number" || ref === "string" || ref === "boolean") {
      return new SqlFragment("?", [expr]);
    }
    switch (expr.type) {
      case "literal":
        return new SqlFragment("?", [expr.value]);
      case "op":
        return this.compileOpExpr(expr, aliases, ctes);
      case "field":
        if (aliases[expr.tableAlias] == null) {
          throw new Error("Alias " + expr.tableAlias + " unknown");
        }
        if (aliases[expr.tableAlias] === true) {
          if (expr.column) {
            if (!expr.column.match(/^[a-z][a-z0-9_]*$/)) {
              throw new Error("Invalid column " + expr.column);
            }
            return new SqlFragment(this.schemaMap.mapTableAlias(expr.tableAlias)).append('."').append(expr.column).append('"');
          } else {
            return new SqlFragment(this.schemaMap.mapTableAlias(expr.tableAlias));
          }
        }
        return this.schemaMap.mapColumn(aliases[expr.tableAlias], expr.column, this.schemaMap.mapTableAlias(expr.tableAlias));
      case "scalar":
        return this.compileScalar(expr, aliases, ctes);
      case "token":
        if ((ref1 = expr.token) === "!bbox!" || ref1 === "!scale_denominator!" || ref1 === "!pixel_width!" || ref1 === "!pixel_height!") {
          return new SqlFragment(expr.token);
        }
        throw new Error("Unsupported token " + expr.token);
        break;
      case "case":
        return this.compileCaseExpr(expr, aliases, ctes);
      default:
        throw new Error("Unsupported type " + expr.type + " in " + (JSON.stringify(expr)));
    }
  };

  JsonqlCompiler.prototype.compileOpExpr = function(expr, aliases, ctes) {
    var compiledExprs, frag, functions, inner, ref, ref1;
    if (ctes == null) {
      ctes = {};
    }
    functions = ["avg", "min", "max", "sum", "count", "stdev", "stdevp", "var", "varp", "row_number", "left", "right", "substr", "lpad", "rpad", "width_bucket", "ntile", "coalesce", "to_json", "to_jsonb", "to_char", "convert_to_decimal", "json_build_array", "json_build_object", "jsonb_build_array", "jsonb_build_object", "json_array_length", "jsonb_array_length", "json_object", "json_array_elements", "jsonb_array_elements", "json_array_elements_text", "jsonb_array_elements_text", "json_typeof", "jsonb_typeof", "array_to_string", "array_agg", "lower", "upper", "round", "ceiling", "floor", "date_part", "json_strip_nulls", "jsonb_strip_nulls", "cos", "sin", "nullif", "log", "ln", "unnest"];
    switch (expr.op) {
      case ">":
      case "<":
      case ">=":
      case "<=":
      case "=":
      case "<>":
      case "/":
      case "~":
      case "~*":
      case "like":
      case "ilike":
      case "&&":
      case "->>":
      case "#>>":
      case "@>":
      case '->':
      case '#>':
      case 'in':
      case '?|':
      case "?&":
        frag = new SqlFragment("(").append(this.compileExpr(expr.exprs[0], aliases, ctes)).append(new SqlFragment(" " + expr.op + " "));
        if ((ref = expr.modifier) === 'any' || ref === 'all') {
          frag.append(expr.modifier).append("(").append(this.compileExpr(expr.exprs[1], aliases, ctes)).append("))");
        } else {
          frag.append(this.compileExpr(expr.exprs[1], aliases, ctes)).append(")");
        }
        return frag;
      case "and":
      case "or":
      case "+":
      case "-":
      case "*":
      case "||":
        compiledExprs = _.map(expr.exprs, (function(_this) {
          return function(e) {
            return _this.compileExpr(e, aliases, ctes);
          };
        })(this));
        compiledExprs = _.filter(compiledExprs, function(e) {
          return !e.isEmpty();
        });
        if (compiledExprs.length === 0) {
          return new SqlFragment();
        } else if (compiledExprs.length === 1) {
          return compiledExprs[0];
        } else {
          inner = SqlFragment.join(compiledExprs, " " + expr.op + " ");
          return new SqlFragment("(").append(inner).append(")");
        }
        break;
      case "is null":
      case "is not null":
        return new SqlFragment("(").append(this.compileExpr(expr.exprs[0], aliases, ctes)).append(new SqlFragment(" " + expr.op)).append(")");
      case "not":
        return new SqlFragment("(not ").append(this.compileExpr(expr.exprs[0], aliases, ctes)).append(")");
      case "between":
        return new SqlFragment("(").append(this.compileExpr(expr.exprs[0], aliases, ctes)).append(" between ").append(this.compileExpr(expr.exprs[1], aliases, ctes)).append(" and ").append(this.compileExpr(expr.exprs[2], aliases, ctes)).append(")");
      case "::text":
      case "::geometry":
      case "::geography":
      case "::uuid":
      case "::integer":
      case "::decimal":
      case "::date":
      case "::timestamp":
      case "::boolean":
      case "::uuid[]":
      case "::text[]":
      case "::json":
      case "::jsonb":
      case "::jsonb[]":
        return new SqlFragment("(").append(this.compileExpr(expr.exprs[0], aliases, ctes)).append(expr.op).append(")");
      case "exists":
        return new SqlFragment("exists (").append(this.compileQuery(expr.exprs[0], aliases, ctes)).append(")");
      case "[]":
        return new SqlFragment("((").append(this.compileExpr(expr.exprs[0], aliases, ctes)).append(")[").append(this.compileExpr(expr.exprs[1], aliases, ctes)).append("])");
      case "interval":
        return new SqlFragment("(interval ").append(this.compileExpr(expr.exprs[0], aliases, ctes)).append(")");
      default:
        if ((ref1 = expr.op, indexOf.call(functions, ref1) >= 0) || expr.op.match(/^ST_[a-zA-z]+$/) || expr.op.match(/^CDB_[a-zA-z]+$/)) {
          inner = SqlFragment.join(_.map(expr.exprs, (function(_this) {
            return function(e) {
              return _this.compileExpr(e, aliases, ctes);
            };
          })(this)), ", ");
          if (expr.op === "count" && inner.isEmpty()) {
            inner = "*";
          }
          if (expr.orderBy) {
            inner = inner.append(this.compileOrderBy(expr.orderBy, aliases));
          }
          if (expr.modifier === "distinct") {
            inner = new SqlFragment("distinct ").append(inner);
          }
          frag = new SqlFragment(expr.op + "(").append(inner).append(")");
          if (expr.over) {
            frag = new SqlFragment("(").append(frag);
            frag.append(" over (");
            if (expr.over.partitionBy) {
              frag.append("partition by ");
              frag.append(SqlFragment.join(_.map(expr.over.partitionBy, (function(_this) {
                return function(pb) {
                  return _this.compileExpr(pb, aliases, ctes);
                };
              })(this)), ", "));
            }
            if (expr.over.orderBy) {
              frag.append(this.compileOrderBy(expr.over.orderBy, aliases));
            }
            frag.append("))");
          }
          return frag;
        }
        throw new Error("Unsupported op " + expr.op);
    }
  };

  JsonqlCompiler.prototype.compileScalar = function(query, aliases, ctes) {
    var f, frag, from, i, len, ref, w, where, withClauses;
    if (ctes == null) {
      ctes = {};
    }
    frag = new SqlFragment('(');
    aliases = _.clone(aliases);
    ctes = _.clone(ctes);
    if (query.withs && query.withs.length > 0) {
      withClauses = [];
      ref = query.withs;
      for (i = 0, len = ref.length; i < len; i++) {
        w = ref[i];
        f = new SqlFragment('"').append(this.schemaMap.mapTableAlias(w.alias));
        f.append("\" as (");
        f.append(this.compileQuery(w.query, aliases));
        f.append(")");
        withClauses.push(f);
        if (ctes[w.alias]) {
          throw new Error("CTE alias " + w.alias + " in use");
        }
        ctes[w.alias] = true;
      }
      frag.append("with ");
      frag.append(SqlFragment.join(withClauses, ", "));
      frag.append(" ");
    }
    frag.append('select ');
    from = this.compileFrom(query.from, aliases, ctes);
    frag.append(this.compileExpr(query.expr, aliases, ctes));
    frag.append(" from ");
    frag.append(from);
    if (query.where) {
      where = this.compileExpr(query.where, aliases, ctes);
      if (!where.isEmpty()) {
        frag.append(" where ");
        frag.append(where);
      }
    }
    if (query.orderBy) {
      frag.append(this.compileOrderBy(query.orderBy, aliases));
    }
    if (query.limit != null) {
      if (!isInt(query.limit)) {
        throw new Error("Invalid limit");
      }
      frag.append(" limit ").append(new SqlFragment("?", [query.limit]));
    }
    if (query.offset != null) {
      if (!isInt(query.offset)) {
        throw new Error("Invalid offset");
      }
      frag.append(" offset ").append(new SqlFragment("?", [query.offset]));
    }
    frag.append(")");
    return frag;
  };

  JsonqlCompiler.prototype.compileCaseExpr = function(expr, aliases, ctes) {
    var c, frag, i, len, ref;
    if (ctes == null) {
      ctes = {};
    }
    frag = new SqlFragment('case ');
    if (expr.input != null) {
      frag.append(this.compileExpr(expr.input, aliases, ctes));
      frag.append(" ");
    }
    ref = expr.cases;
    for (i = 0, len = ref.length; i < len; i++) {
      c = ref[i];
      frag.append("when ");
      frag.append(this.compileExpr(c.when, aliases, ctes));
      frag.append(" then ");
      frag.append(this.compileExpr(c.then, aliases, ctes));
      frag.append(" ");
    }
    if (expr["else"] != null) {
      frag.append("else ");
      frag.append(this.compileExpr(expr["else"], aliases, ctes));
      frag.append(" ");
    }
    return frag.append("end");
  };

  JsonqlCompiler.prototype.validateAlias = function(alias) {
    if (!alias.match(/^[_a-zA-Z][a-zA-Z_0-9. :]*$/)) {
      throw new Error("Invalid alias '" + alias + "'");
    }
  };

  return JsonqlCompiler;

})();

isInt = function(x) {
  return typeof x === 'number' && (x % 1) === 0;
};
