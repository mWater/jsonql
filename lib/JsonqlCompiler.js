...
module.exports = JsonqlCompiler = (function() {
  ...
  JsonqlCompiler.prototype.compileQuery = function(query) {
    var frag, from, selects, where;
    frag = new SqlFragment('select ');
    from = this.compileFrom(query.from);
    selects = _.map(query.selects, (function(_this) {
      return function(s) {
        return _this.compileSelect(s, from.aliases);
      };
    })(this));
    ...
    if (query.where) {
      where = this.compileExpr(query.where, from.aliases);
      if (!where.isEmpty()) {
        frag.append(" where ");
        frag.append(where);
      }
    ...
    if (query.orderBy) {
      frag.append(this.compileOrderBy(query.orderBy, from.aliases));
    }
    if (query.limit != null) {
  ...
  JsonqlCompiler.prototype.compileFrom = function(from) {
    var aliases, left, onSql, ref, right;
    aliases = {};
    switch (from.type) {
      case "table":
        ...
        return {
          sql: this.schemaMap.mapTable(from.table).append(new SqlFragment(' as "' + this.schemaMap.mapTableAlias(from.alias) + '"')),
          aliases: aliases
        };
      case "join":
        left = this.compileFrom(from.left);
        right = this.compileFrom(from.right);
        ...
        return {
          sql: new SqlFragment("(").append(left.sql).append(" " + from.kind + " join ").append(right.sql).append(" on ").append(onSql).append(")"),
          aliases: aliases
        };
      default:
  ...
  JsonqlCompiler.prototype.compileOpExpr = function(expr, aliases) {
    ...
    switch (expr.op) {
      ...
      case "~*":
      case "like":
        return new SqlFragment("(").append(this.compileExpr(expr.exprs[0], aliases)).append(new SqlFragment(" " + expr.op + " ")).append(this.compileExpr(expr.exprs[1], aliases)).append(")");
      case "and":
      case "or":
      ...
      default:
        if (ref = expr.op, indexOf.call(functions, ref) >= 0) {
          inner = SqlFragment.join(_.map(expr.exprs, (function(_this) {
            return function(e) {
              return _this.compileExpr(e, aliases);
            };
