...
class JsonqlCompiler {
    ...
    compileQuery(query, aliases = {}, ctes = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let from;
            if (query.type === "union") {
                const subqueries = yield Promise.all(query.queries.map(q => this.compileQuery(q, aliases, ctes)));
                return SqlFragment_1.default.join(lodash_1.default.map(subqueries, (q) => {
                    return new SqlFragment_1.default("(").append(q).append(")");
                }), " union ");
            }
            ...
            if (query.withs && query.withs.length > 0) {
                ...
                for (let w of query.withs) {
                    ...
                    f.append('" as (');
                    f.append(yield this.compileQuery(w.query, aliases));
                    f.append(")");
                    withClauses.push(f);
                    // Add to cte tables
                    if (ctes[w.alias]) {
            ...
            if (query.from) {
                from = yield this.compileFrom(query.from, aliases, ctes);
            }
            else {
                from = null;
            }
            // Compile selects
            const selects = yield Promise.all(lodash_1.default.map(query.selects, (s) => this.compileSelect(s, aliases, ctes)));
            // Handle null select
            if (selects.length === 0) {
                frag.append("null");
            }
            ...
            // Add where
            if (query.where != null) {
                const where = yield this.compileExpr(query.where, aliases, ctes);
                if (!where.isEmpty()) {
                    frag.append(" where ");
                    frag.append(where);
                }
            ...
            if (query.groupBy) {
                ...
                const groupByExprs = yield Promise.all(lodash_1.default.map(query.groupBy, (groupBy) => {
                    if (isInt(groupBy)) {
                        return new SqlFragment_1.default(`${groupBy}`);
                    }
                    return this.compileExpr(groupBy, aliases, ctes);
                }));
                frag.append(SqlFragment_1.default.join(groupByExprs, ", "));
            }
            // Add order by
            if (query.orderBy) {
                frag.append(yield this.compileOrderBy(query.orderBy, aliases));
            }
            // Add limit
            if (query.limit != null) {
                // Check that is int
    ...
    compileSelect(select, aliases, ctes = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const frag = yield this.compileExpr(expr, aliases, ctes);
            frag.append(" as ");
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            switch (from.type) {
                case "table":
                    ...
                    const mappedTable = yield this.schemaMap.mapTable(from.table);
                    return mappedTable
                        .append(new SqlFragment_1.default(' as "' + this.schemaMap.mapTableAlias(from.alias) + '"'));
                case "join":
                    // Compile left and right
                    var left = yield this.compileFrom(from.left, aliases, ctes);
                    var right = yield this.compileFrom(from.right, aliases, ctes);
                    ...
                    // Compile on
                    var onSql = from.on ? yield this.compileExpr(from.on, aliases, ctes) : undefined;
                ...
                case "subquery":
                    ...
                    // Compile query
                    var subquery = yield this.compileQuery(from.query, aliases, ctes);
                ...
                case "subexpr":
                    ...
                    // Compile expression
                    var subexpr = yield this.compileExpr(from.expr, aliases, ctes);
                    // Record alias as true to allow any field to be queried
                    aliases[from.alias] = true;
    ...
    compileOrderBy(orderBy, aliases) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (orderBy.length > 0) {
                const orderByExprs = yield Promise.all(lodash_1.default.map(orderBy, (o) => __awaiter(this, void 0, void 0, function* () {
                    let f;
                    if (lodash_1.default.isNumber(o.ordinal)) {
                        f = new SqlFragment_1.default(`${o.ordinal}`);
                    }
                    else {
                        f = yield this.compileExpr(o.expr, aliases);
                    }
                    if (o.direction) {
                ...
                frag.append(" order by ").append(SqlFragment_1.default.join(orderByExprs, ", "));
            }
            return frag;
        });
    }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            switch (expr.type) {
                ...
                case "field":
                    ...
                    return yield this.schemaMap.mapColumn(aliases[expr.tableAlias], expr.column, this.schemaMap.mapTableAlias(expr.tableAlias));
                case "scalar":
    ...
    compileOpExpr(expr, aliases, ctes = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            switch (expr.op) {
                ...
                case "?&":
                    var frag = new SqlFragment_1.default("(")
                        .append(yield this.compileExpr(expr.exprs[0], aliases, ctes))
                        .append(new SqlFragment_1.default(" " + expr.op + " "));
                    if (["any", "all"].includes(expr.modifier)) {
                        frag.append(expr.modifier).append("(").append(yield this.compileExpr(expr.exprs[1], aliases, ctes)).append("))");
                    }
                    else {
                        frag.append(yield this.compileExpr(expr.exprs[1], aliases, ctes)).append(")");
                    }
                    return frag;
                case "and":
                case "or":
                ...
                case "*":
                case "||":
                    var compiledExprs = yield Promise.all(lodash_1.default.map(expr.exprs, (e) => this.compileExpr(e, aliases, ctes)));
                    // Remove blanks
                ...
                case "is not null":
                    return new SqlFragment_1.default("(")
                        ...
                        .append(")");
                case "not":
                    return new SqlFragment_1.default("(not ").append(yield this.compileExpr(expr.exprs[0], aliases, ctes)).append(")");
                case "between":
                    return new SqlFragment_1.default("(")
                        .append(yield this.compileExpr(expr.exprs[0], aliases, ctes))
                        .append(" between ")
                        .append(yield this.compileExpr(expr.exprs[1], aliases, ctes))
                        .append(" and ")
                        .append(yield this.compileExpr(expr.exprs[2], aliases, ctes))
                        .append(")");
                case "::text":
                case "::geometry":
                case "::geography":
                ...
                case "::numeric":
                case "::integer[]":
                    return new SqlFragment_1.default("(").append(yield this.compileExpr(expr.exprs[0], aliases, ctes)).append(expr.op).append(")");
                case "exists":
                    return new SqlFragment_1.default("exists (").append(yield this.compileQuery(expr.exprs[0], aliases, ctes)).append(")");
                case "[]":
                    return new SqlFragment_1.default("((")
                        .append(yield this.compileExpr(expr.exprs[0], aliases, ctes))
                        .append(")[")
                        ...
                        .append("])");
                case "interval":
                    return new SqlFragment_1.default("(interval ").append(yield this.compileExpr(expr.exprs[0], aliases, ctes)).append(")");
                case "at time zone":
                    return new SqlFragment_1.default("(")
                        .append(yield this.compileExpr(expr.exprs[0], aliases, ctes))
                        .append(" at time zone ")
                ...
                default:
                    ...
                    if (functions.includes(expr.op) ||
                        ...
                        inner = SqlFragment_1.default.join(yield Promise.all(lodash_1.default.map(expr.exprs, (e) => this.compileExpr(e, aliases, ctes))), ", ");
                        ...
                        // Handle orderBy
                        if (expr.orderBy) {
                            inner = inner.append(yield this.compileOrderBy(expr.orderBy, aliases));
                        }
                        ...
                        if (expr.over) {
                            ...
                            if (expr.over.partitionBy) {
                                ...
                                frag.append(SqlFragment_1.default.join(yield Promise.all(lodash_1.default.map(expr.over.partitionBy, (pb) => this.compileExpr(pb, aliases, ctes))), ", "));
                            }
                            if (expr.over.orderBy) {
                                frag.append(yield this.compileOrderBy(expr.over.orderBy, aliases));
                            }
                            frag.append("))");
                        }
                        return frag;
    ...
    compileScalar(query, aliases, ctes = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            frag.append(yield this.compileExpr(query.expr, aliases, ctes));
            // Add from
            if (from) {
                frag.append(" from ");
                frag.append(from);
    ...
    compileCaseExpr(expr, aliases, ctes = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (expr.input != null) {
                frag.append(yield this.compileExpr(expr.input, aliases, ctes));
                frag.append(" ");
            }
            for (let c of expr.cases) {
                frag.append("when ");
                frag.append(yield this.compileExpr(c.when, aliases, ctes));
                frag.append(" then ");
                frag.append(yield this.compileExpr(c.then, aliases, ctes));
                frag.append(" ");
            }
            if (expr.else != null) {
                frag.append("else ");
                frag.append(yield this.compileExpr(expr.else, aliases, ctes));
                frag.append(" ");
            }
